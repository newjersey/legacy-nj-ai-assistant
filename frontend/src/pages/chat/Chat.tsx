  ...
  const makeApiRequestWithoutCosmosDB = async (
    ...
    try {
      ...
      if (response?.body) {
        ...
        let runningText = "";
        let messageProcessingInProgress: boolean = true;
        while (messageProcessingInProgress) {
        ...
        <Stack horizontal className={styles.chatRoot}>
          <div className={`${styles.chatContainer}`}>
            {!messages || messages.length < 1 ? (
              <div className={`display-flex flex-column flex-justify-center ${styles.chatEmptyState}`}>
                <div
